...
/**
 * Constructs base options from environment variables and CI service detection
 * Detects CI service (Travis, CircleCI, Jenkins, etc.) and extracts relevant metadata
 * @param {Object} [cliOptions] - CLI options (verbose, etc.)
 * @returns {Promise<Object>} Options object with CI and git metadata
 * @throws {Error} If git data fetching fails
 */
const getBaseOptions = async (cliOptions = {}) => {
  ...
  if (process.env.CIRCLECI) {
    ...
    options.service_number = process.env.CIRCLE_BUILD_NUM;
  ...
  options.run_at = process.env.COVERALLS_RUN_AT || new Date().toISOString();
  ...
  const coveralls_yaml_conf = (() => {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        logger.debug('No .coveralls.yml file found');
      } else {
        logger.warn('Error reading .coveralls.yml:', err.message);
      }
    }
  })();
...
/**
 * Gets complete options by merging base options, .coveralls.yml, and user options
 * Reads .coveralls.yml if present, merges with base options and user-provided options
 * @param {Object} [_userOptions] - Optional user-provided options to override defaults
 * @param {Object} [cliOptions] - CLI options including positional arguments
 * @returns {Promise<Object>} Complete merged options object
 * @throws {Error} If option fetching fails
 */
const getOptions = async (_userOptions, cliOptions = {}) => {
  const userOptions = _userOptions || {};

  const options = await getBaseOptions(cliOptions);

  // Handle positional arguments (filepath)
  const firstNonOptionArgument = cliOptions._?.[0];
